...
Task 4 review fix: filelist progress totals exclude discarded next-level tasks
...
实现 Task 4 “Level-Based Filelist Discovery Scheduler”，并修复 review 发现的进度统计问题：当 `filelist_task_limit_per_bucket` 已达到、下一层候选目录不会被实际扫描时，scan.log 和 tqdm 的 total 不能把这些被丢弃的候选任务计入总任务数。
...
- 无代码待办；等待 Task 4 review fix 提交、push 后继续 Task 5。
...
- 这次只覆盖了 Task 4 brief 指定的 scanner 行为及 review 发现的进度 total 边界；更大范围的端到端调度联动仍依赖后续任务或更广测试验证。
- 本地 `.superpowers/sdd/task-4-report.md` 必须保持未跟踪状态，不能进入 commit。

## Next recommended action

- 提交并 push Task 4 review fix，然后重新请求 Task 4 reviewer 确认。
