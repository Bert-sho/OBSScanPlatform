...
Python 3.11+ package under `src/obs_scan_platform/`. It scans OBS buckets over an HTTP API and produces configurable per-bucket CSV or Parquet overviews. The CLI (`cli.py`) and FastAPI server (`api.py`) are thin fronts over `scanner.run_scan`.
...
5. The configured aggregator turns the temp per-object rows into either the legacy directory CSV or the bounded Parquet overview.

### Filelist discovery scheduler (`filelist_discovery.py`)
`FilelistDiscoveryScheduler` runs level-by-level BFS starting at `/`, capped by `Thresholds.filelist_depth` and `scan.filelist_task_limit_per_bucket`. Before each BFS level, it checkpoints the current prefix frontier and metadata candidates. The task limit gates whether to descend to the next level; it does not truncate already-discovered tasks at the current level. After a complete level, a cumulative metadata task count above `scan.metadata_task_limit_per_bucket` rolls back that whole level; `/` is the root fallback when the first level overflows. Successfully expanded directories leave the final prefix set, so `objectkeys` scans only the non-overlapping traversal frontier. Empty directories are removed from the rollback checkpoint, while failed directories remain frontier prefixes; `record_failed` also prunes descendants and direct files discovered on earlier pages.
...
### Data model & aggregation (`models.py`, `paths.py`, `csv_store.py`, `aggregation.py`, `parquet_aggregation.py`)
Objects are written as top-level temp detail CSVs under `results/<run_id>/_tmp/<appid>/<bucket>/`: one per prefix plus `metadata_files.csv` when present. With `scan.overview_format=csv`, `aggregation.aggregate_bucket` preserves the legacy pure-CSV external merge and ancestor rollup, writing `results/<run_id>/<appid>/<bucket>.csv`. With `scan.overview_format=parquet` (the default), `parquet_aggregation.aggregate_bucket_parquet` attributes every object to exactly one directory, truncates paths deeper than `scan.aggregation_depth` (default `4`) into that cutoff directory, maps file extensions through the default-plus-override `scan.file_type_map`, and writes Snappy-compressed parts of at most 50,000 rows to `results/<run_id>/<appid>/<bucket>/part-NNNNN.parquet`. Parquet rows are sorted by path; their `max_depth` is the deepest original containing-directory level represented by the row, excluding the filename. Rows use the UTC scan-start date when every aggregated object lacks a valid modification timestamp. The external reducers use a fan-in of 32 and bounded per-run metadata; aggregation does not deduplicate, so every duplicate detail row is counted once per occurrence.

Aggregation artifacts live under `_aggregation/chunks`, `_aggregation/prefixes`, and `_aggregation/bucket-runs`. With `scan.keep_temp_files=false`, consumed runs may be deleted early and every bucket finalizes its temporary directory immediately for `success`, `partial_failed`, and `failed` results while still holding its run-global bucket permit. With `true`, detail CSVs and all aggregation artifacts are retained, potentially using substantial disk. A cleanup error converts that bucket result to `failed` without abandoning sibling scans. Aggregation remains in the existing processing timing phase. Status uses `_rollup_status` (`success`/`partial_failed`/`failed`); partial failures are collected in `PartialErrorSummary`. The manifest records `overview_format`, `overview_path`, and `overview_files`; CSV results retain `csv_path`, while Parquet parts are downloaded through the manifest-authorized per-part API route.

### Config (`config.py`)
Single `AppConfigFile` loaded from YAML, with model defaults for every field. A non-empty per-application `endpoint` takes precedence; otherwise a missing, empty, or null application endpoint inherits a non-empty global endpoint. Global `scan.overview_format` selects `parquet` (default) or `csv`; `scan.aggregation_depth` defaults to `4`, and `scan.file_type_map` merges normalized YAML overrides over the built-in extension categories. Legacy input `scan.max_depth` migrates only when the canonical name is absent; providing both names fails validation, and serialization exposes only `aggregation_depth`. `thresholds_for` merges global defaults (`large_directory_bytes=107374182400`, `large_file_bytes=10737418240`, `inactive_directory_days=180`, `filelist_depth=5`) with nullable per-bucket `BucketOverrides`. `BucketOverrides.enable` defaults to `true`, and only explicit `false` opts out an exact bucket name. `missing_scan_fields` enforces resolved endpoint/appid/apptoken at application scan time. `masked_dict` masks `apptoken` for API responses.
