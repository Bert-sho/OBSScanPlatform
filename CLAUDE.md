...
Objects are written as top-level temp detail CSVs under `results/<run_id>/_tmp/<appid>/<bucket>/`: one per prefix plus `metadata_files.csv` when present. `aggregation.aggregate_bucket` uses a pure-CSV external merge. It processes one detail source at a time; `directory_chain_for_object` attributes each row to its containing directory and every ancestor, and the current `DirectoryStats` dictionary is flushed to a sorted chunk when it reaches `scan.aggregation_max_directories_in_memory` (default `100000`, minimum `1`). Because one object's full ancestor chain is added before the check, the configured directory count can be exceeded by at most that object's directory depth; it is not a byte-accurate memory bound. Each completed chunk immediately enters an online hierarchical reducer to produce its sorted source summary, and each completed source summary immediately enters the bucket reducer. Both reducers have a fixed maximum fan-in of 32 and retain only bounded per-level path metadata before streaming the unchanged final schema to `results/<run_id>/<appid>/<bucket>.csv` through an atomic temporary-file replacement. No object-key set is retained and aggregation does not deduplicate: every duplicate detail row is counted once per occurrence.
