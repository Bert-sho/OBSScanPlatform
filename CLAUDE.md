...
`FilelistDiscoveryScheduler` runs level-by-level BFS starting at `/`, capped by `Thresholds.filelist_depth` and `scan.filelist_task_limit_per_bucket`. The task limit gates whether to descend to the next level; it does not truncate already-discovered tasks at the current level. `rollback_failed_task` prunes a failed directory's entire subtree (prefixes, direct files, queued tasks) so partial failures don't leave orphaned prefixes — this is delicate; see recent git history. `record_empty` drops prefixes that turned out empty.
